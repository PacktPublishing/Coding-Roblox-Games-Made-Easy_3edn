...
local function getPlayerInTable(player)
	for i, competitor in competitors do
...
local function spawnPlayers()
	...
	for _, player in competitors do
		...
		char:PivotTo(spawnPoint.CFrame * CFrame.new(0,2,0))
	end
end
...
gameRunner.gameLoop = function()
	while task.wait(0.5) do
		if #playerService:GetPlayers() < MIN_PLAYERS then
			...
			repeat
				message.Value = "Time remaining: ".. shared.mod("Utilities").formatTimeRemaining(gameTime, true)
